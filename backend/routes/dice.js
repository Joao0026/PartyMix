...
router.post('/', async (req, res) => { try { res.status(201).json(await new D(req.body).save()); } catch (e) { res.status(400).json({ error: e.message }); } });
router.delete('/:id', async (req, res) => { try { await D.findByIdAndDelete(req.params.id); res.json({ deleted: true }); } catch (e) { res.status(500).json({ error: e.message }); } });
